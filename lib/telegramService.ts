...
export function formatTelegramAnalysisMessage(analysis: AnalysisResult, currentPrice?: number): string {
  ...
  const lines = [
    ...
    mtfRec ? `   └─ ${mtfRec}` : "",
    ``,
    analysis.tradeSetup.dealingRangeZone ? `🏛️ <b>SMC Dealing Range:</b> <code>${analysis.tradeSetup.dealingRangeZone} (${analysis.tradeSetup.dealingRangePercentile ?? 50}%)</code>` : "",
    analysis.tradeSetup.isPostNewsSniper ? `🎯 <b>กลยุทธ์:</b> <code>⚡ POST-NEWS SNIPER (Turtle Soup Liquidity Sweep)</code>` : "",
